...
func generate_world():
	...
	for x in range(stage_data.map_width):
		for y in range(stage_data.map_height):
			...
			var eh_borda = (x == 0 or x == stage_data.map_width - 1 or
		y == 0 or y == stage_data.map_height - 1)
			if eh_borda:
				if qtd_obstaculos > 0:
					var s_id = stage_data.obstaculos_source_ids[0]
					var atlas = stage_data.obstaculos_atlas[0]
